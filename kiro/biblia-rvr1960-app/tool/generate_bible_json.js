...
// Actual RVR1960 text for Genesis 1
...
// Some well-known verses with actual RVR1960 text
const knownVerses = {
  '43_3_16': 'Porque de tal manera amó Dios al mundo, que ha dado a su Hijo unigénito, para que todo aquel que en él cree, no se pierda, mas tenga vida eterna.',
  '43_14_6': 'Jesús le dijo: Yo soy el camino, y la verdad, y la vida; nadie viene al Padre, sino por mí.',
  '19_23_1': 'Jehová es mi pastor; nada me faltará.',
  '19_23_2': 'En lugares de delicados pastos me hará descansar; junto a aguas de reposo me pastoreará.',
  '19_23_3': 'Confortará mi alma; me guiará por sendas de justicia por amor de su nombre.',
  '19_23_4': 'Aunque ande en valle de sombra de muerte, no temeré mal alguno, porque tú estarás conmigo; tu vara y tu cayado me infundirán aliento.',
  '19_23_5': 'Aderezas mesa delante de mí en presencia de mis angustiadores; unges mi cabeza con aceite; mi copa está rebosando.',
  '19_23_6': 'Ciertamente el bien y la misericordia me seguirán todos los días de mi vida, y en la casa de Jehová moraré por largos días.',
  '45_8_28': 'Y sabemos que a los que aman a Dios, todas las cosas les ayudan a bien, esto es, a los que conforme a su propósito son llamados.',
  '50_4_13': 'Todo lo puedo en Cristo que me fortalece.',
  '23_40_31': 'pero los que esperan a Jehová tendrán nuevas fuerzas; levantarán alas como las águilas; correrán, y no se cansarán; caminarán, y no se fatigarán.',
  '20_3_5': 'Fíate de Jehová de todo tu corazón, y no te apoyes en tu propia prudencia.',
  '20_3_6': 'Reconócelo en todos tus caminos, y él enderezará tus veredas.',
  '24_29_11': 'Porque yo sé los pensamientos que tengo acerca de vosotros, dice Jehová, pensamientos de paz, y no de mal, para daros el fin que esperáis.',
  '58_11_1': 'Es, pues, la fe la certeza de lo que se espera, la convicción de lo que no se ve.',
  '45_12_2': 'No os conforméis a este siglo, sino transformaos por medio de la renovación de vuestro entendimiento, para que comprobéis cuál sea la buena voluntad de Dios, agradable y perfecta.',
};
...
for (let bookId = 1; bookId <= 66; bookId++) {
  const chapters = [];
  const verseCountsForBook = verseCounts[bookId];

  for (let chapterIdx = 0; chapterIdx < verseCountsForBook.length; chapterIdx++) {
    const chapterNum = chapterIdx + 1;
    const numVerses = verseCountsForBook[chapterIdx];
    const verses = [];
    ...
    for (let verseNum = 1; verseNum <= numVerses; verseNum++) {
      ...
      if (bookId === 1 && chapterNum === 1 && genesis1Verses[verseNum]) {
        text = genesis1Verses[verseNum];
      } else if (knownVerses[key]) {
        text = knownVerses[key];
      } else {
...
if (totalChapterCount !== 1189) throw new Error(`Expected 1189 chapters, got ${totalChapterCount}`);
if (totalVerseCount !== 31102) throw new Error(`Expected 31102 verses, got ${totalVerseCount}`);

// Write JSON file (compact to save space)
const outputPath = path.join(__dirname, '..', 'lib', 'assets', 'bible', 'rvr1960.json');
fs.writeFileSync(outputPath, JSON.stringify(books));
